...
> 4. ==**！！！重要！！！**==
>
>    ==**Legacy support：将在正式版发布的时候剔除第三方指针传递者的实现，请迁移到新的链表实现。**==
...
订阅者应该保存一个订阅者类型的指针`Subscriber_t*`，在初始化的时候调用`SubRegister()`并传入要订阅的事件名和该事件对应消息的长度，可以直接输入字符串，示例如下，将从event_name订阅float数据：
...
**多个publisher可以绑定同一个事件，往该事件推送消息。但一个subscriber只能订阅一个事件，如果应用需要订阅多个事件，则要创建对应数量的订阅者。**

> 对于电控程序目前的情况，不存在多个publisher向同一个事件推送消息的情况。
